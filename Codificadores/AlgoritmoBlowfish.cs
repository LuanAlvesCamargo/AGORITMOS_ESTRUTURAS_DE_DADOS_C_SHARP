...
namespace Algorithms.Encoders;
    ...
    // O algoritmo Blowfish é uma cifra de bloco de chave simétrica, o que significa que usa a mesma chave secreta para criptografar e
    // descriptografar dados. Foi projetado por Bruce Schneier em 1993.

public class BlowfishEncoder {
    // Inicialize modVal para 2 ^ 32
    private const ulong ModVal = 4294967296L;

    // Inicialize as caixas de substituição
    ...
    public string Encrypt(string plainText) {
        // Execute as 16 rodadas do algoritmo baiacu no texto simples.
        for (var i = 0; i < 16; i++) {
            plainText = Round(i, plainText);
        }

        // Troque as partes esquerda e direita do plainText.
        var left = plainText.Substring(8, 8);
        var right = plainText[..8];

        // XOR na metade esquerda com a última subchave do array P.
        left = Xor(left, p[17]);

        // XOR na metade direita com a penúltima subchave do array P.
    ...
    public string Decrypt(string cipherText) {
        // Execute 16 rodadas do algoritmo blowfish no cipherText na ordem inversa.
        for (var i = 17; i > 1; i--) {
            cipherText = Round(i, cipherText);
        }

        // Troque as metades esquerda e direita do cipherText.
        var left = cipherText.Substring(8, 8);
        var right = cipherText.Substring(0, 8);

        // XOR na metade esquerda com a primeira subchave do array P.
        left = Xor(left, p[0]);

        // XOR na metade direita com a segunda subchave do array P.
        right = Xor(right, p[1]);

        // Retorne a string descriptografada como uma string concatenada.
        return left + right;
    }
    ...
    private string HexadecimalToBinary(string hex) {
        return hex.Select(t =>
                ...
                // Converta cada caractere em um número inteiro usando base 16
                Convert.ToString(Convert.ToInt32(t.ToString(), 16), 2))

            // Preencha cada string binária com zeros à esquerda para torná-la com 4 bits de comprimento
            .Select(fourBitBinary => fourBitBinary.PadLeft(4, '0'))

            // Concatene todas as strings binárias em uma
    ...
    private string BinaryToHexadecimal(string binaryInput) {
        return string.Concat(
            Enumerable.Range(0, binaryInput.Length / 4)
                ...
                // Selecione cada grupo de 4 bits
                .Select(index => binaryInput.Substring(index * 4, 4))

                // Converta cada grupo em um número inteiro usando base 2
                .Select(fourBitBinary => Convert.ToInt32(fourBitBinary, 2)

                    // Converta cada número inteiro em um caractere hexadecimal usando base 16
                    .ToString("x")));
    }

    private string Xor(string left, string right) {
        // Converte as strings hexadecimais em strings binárias usando um método auxiliar
        left = HexadecimalToBinary(left);
        right = HexadecimalToBinary(right);

        var xor = new StringBuilder();

        // Loop através de cada bit nas strings binárias
        for (var i = 0; i < left.Length; i++)
        {
            // Execute uma operação XOR bit a bit nos bits correspondentes e anexe o resultado a xor
            xor.Append((char)(((left[i] - '0') ^ (right[i] - '0')) + '0'));
        }

        //  Converta a string binária em uma string hexadecimal
        var result = BinaryToHexadecimal(xor.ToString());
        return result;
    }

    private string AddAndMod(string left, string right) {
        // Converta as strings hexadecimais em inteiros não assinados de 64 bits usando base 16
        var leftNumber = Convert.ToUInt64(left, 16);
        var rightNumber = Convert.ToUInt64(right, 16);

        // Some os dois inteiros e calcule o restante após dividir por _modVal
        var total = (leftNumber + rightNumber) % ModVal;

        // Converta o resultado em uma string hexadecimal usando base 16
        var result = total.ToString("x");

        // Preencha o resultado com zeros à esquerda para torná-lo com 8 caracteres
        result = "00000000" + result;

        // Retorne os últimos 8 caracteres do resultado
        return result[^8..];
    }
    ...
    private string Round(int feistelRound, string plainText) {
        // Divida o plainText em duas metades de 32 bits.
        var left = plainText[..8];
        var right = plainText.Substring(8, 8);

        // XOR na metade esquerda com a subchave do array P.
        left = Xor(left, p[feistelRound]);

        // Aplique a função F à metade esquerda.
        var fOutput = F(left);

        // XOR a saída da função F com a metade direita.
        right = Xor(fOutput, right);

        // Troque as metades esquerda e direita e retorne-as como uma string concatenada.
        return right + left;
    }
}
